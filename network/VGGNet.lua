...
local VGGNet = torch.class('tracking.VGGNet')
--local utils = detection.GeneralUtils()
function VGGNet:__init(model_opt)
    -- rnn opts
...
function VGGNet:load_weight(weight_file_path)
...
function VGGNet:getCNNParameters()
    return self.cnnw, self.cnndw
end

function VGGNet:training()
	self.cnn:training()
    self.classifier:training()
    --self.fusionnet:training()
end

function VGGNet:evaluate()
	self.cnn:evaluate()
    self.classifier:evaluate()
end

function VGGNet:save(save_path)
...
function VGGNet:load_from_caffe(proto_path,caffemodel_path,save_path,model_name)
...
function VGGNet:get_model()
	return self.model
end


function VGGNet:cnnforward(inputs)
...
function VGGNet:cnnbackward(inputs, targets)
    --inputs[t] {features, cnnInput {pre_img,rois = {id = boxinfo{box,gt}}}
    self.cnndw:zero()
...
function VGGNet:_sanitize(network)
