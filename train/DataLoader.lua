...
print(string.format('has dets %d',#self.det_data))
    ...
    if self.train then
     ...
     print('process gt and det')
    for fr = 1, self._imgs do
      self.Det[fr] = self:processBboxes(self.det_data[fr])
      if self.train == false then
        self.Gt[fr] = self.Det[fr]
      else
...
function DataLoader:getNextState()
  ...
  for id,box in pairs(self.Gt[self._cur_image]) do
    ...
    self.cur_state.rois[id] = boxInfo
  ...
  --  local imSize = self.imgs[1]:size()
    ...
    --local imSize = self.imgs[1]:size()
    --[[local randBoxes = randomBbox(20,imSize,self.Gt[self._cur_image])
    for id ,box in pairs(randBoxes) do
      local boxInfo = {}
      boxInfo.box = box:clone()
      boxInfo.gt  = 2
      self.cur_state.rois[maxId+id] = boxInfo
    end]]
  --end
  --local imSize = self.imgs[1]:size()
  --local randBoxes = randomBbox(20,imSize,self.Gt[self._cur_image])
  self.cur_state._rois = count
  if count == 0 then
    self:getNextState()
  else
